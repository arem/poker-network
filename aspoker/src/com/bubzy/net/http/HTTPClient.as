...
import flash.utils.*;
	...
	{	
		...
		private var _httpResponseHeaders:Array = new Array;
		...
		private function _connect():Boolean
		{
			...
			Security.loadPolicyFile("xmlsocket://" + _httpRequestServer + ":843");
		...
		private function _initResponse():void
		{
			_bytesLoaded = 0;
			_bytesTotal = 0;
			_httpResponseHeaders = new Array;
        ...
        {
       		var pattern1:RegExp = new RegExp("^HTTP\/(?P<version>1.[0-9]) (?P<statusCode>[1-5][01][0-9]) (?P<statusText>[a-zA-Z0-9._%+-]+)");
        	var results:Array = str.match(pattern1);
			...
			if ( results != null ) {
				...
				dispatchEvent(new HTTPStatusEvent(HTTPStatusEvent.HTTP_STATUS, false, false, _httpResponseStatusCode));
	   			return true;
   			}
   			return false;
        }	
		...
		private function _onSocketDataEvent(event:ProgressEvent):void 
		{
			...
			var response:Array = buffer.split(HTTP_END_HEADER);
			...
			for each(headerLine in headerArr) {
				_parseResponseHeaderStatus(headerLine); 
				_parseResponseHeader(headerLine);
			}
		...
		{
			_httpRequest.requestHeaders.push(new URLRequestHeader("Host",_httpRequestServer));
			_httpRequest.requestHeaders.push(new URLRequestHeader("Content-Length",_httpRequest.data.length));
			
			if (_httpResponseCookie != "" && _httpResponseCookie != null)  {
				_httpRequest.requestHeaders.push(new URLRequestHeader("Cookie",_httpResponseCookie));
			}
		}
		...
		private function _sendRequest():void
		{
			...
			if(requestHeaders.length > 0) {
				for each(var httpHeader:URLRequestHeader in requestHeaders)	{
					HttpRequest+= httpHeader.name + ":" + httpHeader.value + HTTP_END_LINE;
				}
			}
