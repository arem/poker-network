...
/*
 *  caro2a.c: a variation of caro2 that tries random hole cards that
 *	fit the pattern that Keith Miyake's best entry to date uses.
 *              
...
/*
 * Keith's winning set of hands so far matches the pattern where there are
 * 5 of two suits, and 4 of the other two suits.  The first hand contains
 * an ace and a card less than an 8, the second a king and a card less
 * than an 8, the third a queen and a card less than an eight, down to
 * an eight and a card less than an eight and then two cards whose ranks
 * are less than an 8.
 */

#define EXCHANGE(x, i, j)			\
do						\
{						\
  typeof ((x)[0]) _temp;			\
  						\
...
{
  static int suit_shifts[] = {  D, H, /* 0 1 */
				H, D, /* 2 3 */
				S, H, /* 4 5 */
				D, C, /* 6 7 */
				S, H, /* 8 9 */
