/*
 *  eval_n.h: a faster poker hand evaluator
 *
...
static inline uint32 en_is_flush(cards_u cards, uint32 *flushranks )
{
    eval_u retval;
...
static inline uint32 en_is_straight( uint32 ranks )	/* Keith's */
{
    unsigned int ranks2;
    eval_u retval;
...
static inline uint32 eval_n( cards_u cards, int n_cards )
{
...
#define SS cards.cards_t.spades
    ...
    if (n_ranks >= HAND_SIZE)
      {
        retval.eval_n = en_is_flush(cards, &flushranks);
        if (retval.eval_n) {
          tempeval.eval_n = en_is_straight(flushranks);
          if (tempeval.eval_n) {
            retval.eval_n = 0;
...
/*-->*/     return retval.eval_n;
          ...
          retval.eval_n = en_is_straight(ranks);
      };
